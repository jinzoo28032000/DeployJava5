			...
			<div class="page-breadcrumb">
				<div class="row align-items-center">
					...
					<div class="col-md-6 col-4 align-self-center">
						<div class="text-end">
							<a href="<%=request.getContextPath()%>/admin/product/create"
							...
							<div class="card-body">
								<div class="table-responsive">
									<table class="table stylish-table no-wrap">
										<thead>
											<tr>
												<th class="border-top-0" colspan="1">Ảnh sản phẩm</th>
										...
										<tbody>
											<c:forEach var="item" items="${list}">
												<tr>
													<td><span class=""><img
															src="/images/product/${item.image}" alt="user"
															width="100" /></span></td>
													<td class="align-middle">${item.name}</td>
													<fmt:formatNumber type="number" pattern="###,###,### VNĐ"
														value="${item.price}" var="price" />
													<td class="align-middle">${price}</td>
													<fmt:formatDate value="${item.createdDate}" var="birthday"
														pattern="dd/MM/yyyy" />
													...
													<td><a class="text-white"
														href="<%=request.getContextPath()%>/admin/product/edit/${item.id}">
															<button class="btn-info px-3 py-1 mt-3">Sửa</button>
													</a> <a class="text-white"
														href="<%=request.getContextPath()%>/admin/product/delete/${item.id}">
